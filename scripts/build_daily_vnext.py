...
def build(date_value: str, template_path: Path, output_path: Path, report_path: Path, dry_run: bool = False) -> dict:
    ...
    if not dry_run:
        ...
        flow_head = f'''    <section class="flow-head"><div><h2>今日论文时间流</h2><p class="result-status" data-result-status aria-live="polite">今日共 {len(records)} 项研究内容</p></div><p>更新于 {esc(latest_label)} 北京时间 · 按首次监测时间</p></section>'''
